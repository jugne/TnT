...
public class NodeReheight2 extends Operator {
	public final Input<TransmissionTree> treeInput = new Input<>("tree", "the species tree", Validate.REQUIRED);
...
//			"list of gene trees that constrain species tree movement", new ArrayList<>());
...
//    private int[][] leafNodeMaps;
    ...
    public void initAndValidate() {
		final TransmissionTree speciesTree = treeInput.get();
...
//			leafPositionArrays[i] = new RelativePosition[intervals.get(i).geneTreeTipAssignment.keySet().size()];
    ...
    public double proposal() {
		final TransmissionTree tree = treeInput.get();
		...
		final List<Double> trHeights = Tools.getTransmissionHeights(tree);
		boolean trNode = false;
		if (trHeights.contains(originalHeight))
			trNode = true;
		...
		// for some reason if the root is not reset - even if the root node is the same
		// node as before! - the
		...
		if (trNode) {
			List<GeneTreeIntervals> intervals = geneTreeIntervalsInput.get();
			for (GeneTreeIntervals interval : intervals) {
				Node right = canonicalOrder[chosenNode].getRight();
				List<Integer> geneNodesAtTr = null;
				try {
					geneNodesAtTr = interval.inverseGeneTreeTipAssignment
							.get(canonicalOrder[chosenNode].getRight().getNr());
				} catch (Exception e) {
					System.out.println();
					// TODO: handle exception
				}

				if (geneNodesAtTr != null) {
					for (int n : geneNodesAtTr) {
						Node logicalNode = Pitchforks.getLogicalNode(interval.geneTreeInput.get().getNode(n));
						if (logicalNode.getHeight() == originalHeight) {
//							Node logicalNode = Pitchforks.getLogicalNode(interval.geneTreeInput.get().getNode(n));
							List<Node> logicalChildren = Pitchforks.getLogicalChildren(logicalNode);
							List<Node> nodes = Pitchforks.getGroup(logicalNode);
							if (newHeight >= logicalNode.getParent().getHeight()
									|| newHeight <= logicalChildren.get(0).getHeight()
									|| newHeight <= logicalChildren.get(1).getHeight())
								return Double.NEGATIVE_INFINITY;
