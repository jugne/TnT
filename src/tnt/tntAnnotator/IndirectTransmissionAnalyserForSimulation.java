...
public class IndirectTransmissionAnalyserForSimulation extends TreeAnnotator {
	...
	public IndirectTransmissionAnalyserForSimulation(TntAnalyserOptions options) throws IOException {
...
//			BitSet transmissions = new BitSet(hostsList.size() * hostsList.size());
		...
		private void fillTransmissions(Node subroot, Double[] transmissions,
				HashMap<String, List<Double>> transmissionTimes,
			List<String> hostsList) {
		if (!subroot.isLeaf()) {
			Node donor;
			if (!subroot.isFake()) {
				donor = subroot.getChild(0).metaDataString.contains("orientation=donor") ? subroot.getChild(0)
						: subroot.getChild(1);
			} else {
				donor = Tools.equalHeightWithPrecision(subroot.getChild(0), subroot) ? subroot.getChild(0)
						: subroot.getChild(1);
			}
...
//			String firstLeafDonor = firstLeafId(donor);
			...
			Node parent = recipient.getParent();
			if (subroot.isRoot() && hostsList.contains(subroot.getID().split("_")[0])){
				int idx = hostsList.indexOf(subroot.getID().split("_")[0]);
				idx = idx * hostsList.size() + idx;
				transmissions[idx] = 1.0;
			}

			while (parent != null) {
				String parentId = parent.getID().split("_")[0];
				int idx = hostsList.indexOf(parentId);
				if (idx != -1) {
				int idx2 = hostsList.indexOf(firstLeafRecipient);
				if (idx != idx2) {
					idx = idx * hostsList.size() + idx2;
					transmissions[idx] = 1.0;
					break;
				}
				}
				idx = hostsList.indexOf(parentId);
				parent = parent.getParent();
				if (parent == null) {
						int idx3 = hostsList.indexOf("0");
						idx3 = idx3 * hostsList.size() + hostsList.indexOf(firstLeafRecipient);
						transmissions[idx3] = 1.0;
			...
			fillTransmissions(subroot.getChild(0), transmissions, transmissionTimes, hostsList);
			fillTransmissions(subroot.getChild(1), transmissions, transmissionTimes, hostsList);
		}
	}
...
//	}
	...
	public static void main(String[] args) {
		...
		// Run ACGAnnotator
		try {
			new IndirectTransmissionAnalyserForSimulation(options);
		} catch (Exception e) {
			if (args.length == 0) {
