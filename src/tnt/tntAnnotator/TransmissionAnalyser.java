...
public class TransmissionAnalyser extends TreeAnnotator {
    private static class TransmissionAnalyserOptions extends TreeAnnotator {
    ...
    public TransmissionAnalyser(TransmissionAnalyser.TransmissionAnalyserOptions options) throws IOException {
        // Display options:
        ...
        File infectionTimesOut = new File(outDir, justName+"_infectionTimes.txt");
        ...
        while (trTreeSet.hasNext()) {
            ...
            if (first) {
                ...
                for (int i=0; i<tree.getLeafNodeCount(); i++) {
                    String hostName =tree.getNode(i).getID().split("_")[0];
                    hosts.add(hostName);
                }
            ...
            first = false;
            Integer[] transmissions = new Integer[hostsList.size() * hostsList.size()];
            Arrays.fill(transmissions, 0);

            Double[] infectionTimes = new Double[hostsList.size()-1];
            Arrays.fill(infectionTimes, 0.0);

            if(!tree.getRoot().metaDataString.contains("host")){
                addHostMetadata(tree.getRoot());
//                printUsageAndError("Trees must have host metadata! " +
//                        "Make sure trees are generated by TnT package!");
            }


//            if (!tree1.hasHostMetadata()){
//                tree1.addHostMetadata();
//            }
            List<String> hostsListNoUnsampled = new ArrayList<String>(hostsList);
            ...
            for (Node leaf : tree.getExternalNodes()){
                ...
                if(hostsListNoUnsampled.size()==0)
                    System.out.print("");
                fillInfectionTimes(leaf, infectionTimes, hostsListNoUnsampled);
            }
            ...
            for (int i=0; i<hostsListNoUnsampled.size();i++){
                if(i == hostsListNoUnsampled.size()-1){
                    psInfectionTimes.print(infectionTimes[i]);
                } else {
                    psInfectionTimes.print(infectionTimes[i]+"\t");
                }
            }
    ...
    private void fillTransmissions(Node leaf, Integer[] transmissions,
        ...
        if ( !parent.isFake() && child.metaDataString.contains("recipient")){
            nTransmissions +=1;
        }
        ...
        while(!parent.isRoot() &&
            ...
            if(parent.getChildCount() == 1 || (!parent.isFake() && child.metaDataString.contains("recipient"))){
...
//        if (idx<0){
//            idx = hostsList.indexOf("unsampled");
...
//        }
     ...
     */
    private static boolean getOptionsGUI(TransmissionAnalyser.TransmissionAnalyserOptions options) {
...
//            outFileChooser.setSelectedFile(options.outFile);
     ...
     */
    public static void getCLIOptions(String[] args, TransmissionAnalyser.TransmissionAnalyserOptions options) {
        int i=0;
    ...
    public static void main(String[] args) {
        TransmissionAnalyser.TransmissionAnalyserOptions options = new TransmissionAnalyser.TransmissionAnalyserOptions();
...
//        Node subTreeRoot = getNode(subtreeRootNr);
        ...
        if (subTreeRoot.isLeaf()) {
            subTreeRoot.setMetaData("host", subTreeRoot.getID().split("_")[0]);
        ...
        if (subTreeRoot.getChildCount()==1 && !subTreeRoot.isRoot()){
            ...
            if (parent.getChildCount()>1 && !parent.isFake()){ // coalescent (transmission event)
                if (subTreeRoot.metaDataString.contains("orientation=donor")){
                    subTreeRoot.setMetaData("host", parent.getMetaData("host"));
                    subTreeRoot.metaDataString = parent.metaDataString;
                } else {
                    subTreeRoot.setMetaData("host", "unsampled");
                    subTreeRoot.metaDataString = String.format("%s,%s=%s",
            ...
            Node left = subTreeRoot.getLeft().getMetaData("orientation").equals("donor") ?
                    subTreeRoot.getLeft() : subTreeRoot.getRight();
            subTreeRoot.setMetaData("host", firstLeafId(left));
            subTreeRoot.metaDataString = String.format("%s,%s=%s",
                    metaData, "host", firstLeafId(left)); // bug here
        }
    ...
    private String firstLeafId(Node n) {
        ...
        if (n.isLeaf()) {
            ...
            if (Tools.equalHeightWithPrecision(n, n.getChild(0))) {
                return n.getChild(0).getID().split("_")[0];
            }
            if (n.getChildCount() > 1 && Tools.equalHeightWithPrecision(n, n.getChild(1))) {
                return n.getChild(1).getID().split("_")[0];
